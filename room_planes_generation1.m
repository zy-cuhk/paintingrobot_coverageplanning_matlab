function [room_plane_edge_point_cell,room_plane_edge_cell,room_plane_norm_vector,room_triangle_cell,room_triangle_edge_cell,triangle_norm_vector_cell]=room_planes_generation1(room_facet,room_vertices,room_norm_vector)
...
for i=1:1:size(room_triangle_cell,2)
    m=1;
    for j=1:1:size(room_triangle_edge_cell{i},1)
        adjacent_num=0;
        for k=1:1:size(room_triangle_edge_cell{i},1)
            if j~=k
                if room_triangle_edge_cell{i}(j,1:6)==room_triangle_edge_cell{i}(k,1:6)
                    adjacent_num=adjacent_num+1;
                end
            end
        end
        if adjacent_num==0
            room_plane_edge_cell{i}(m,1:6)=room_triangle_edge_cell{i}(j,1:6);
            m=m+1;
        end
    end
...
% 第四步，将平面边界线转换为平面边界点
% input: room_plane_edge_cell;
% output: room_plane_edge_point_cell;

for i=1:1:size(room_plane_edge_cell,2)
    for j=1:1:size(room_plane_edge_cell{i},1)
        room_plane_edge_point_cell{i}(2*j-1,1:3)=room_plane_edge_cell{i}(j,1:3);
        room_plane_edge_point_cell{i}(2*j,1:3)=room_plane_edge_cell{i}(j,4:6);
    end
    room_plane_edge_point_cell{i}=unique(room_plane_edge_point_cell{i},'rows');
end
end
