function [ground_edge_cells,new_renovation_planes_edge_cell,new_renovation_planes_norm_vector,new_renovation_plane_triangle_edges]=renovation_planes_generation(room_plane_norm_vector,room_plane_edge_cell,room_plane_edge_centroid,room_plane_triangle_edge_cell,panning_distance)
...
%% secondly the processing of planar parameters of room vertical planes
...
%% thirdly, the processing of planar points of vertical planes
...
for i=1:1:size(room_vplane_edge_cell,2)
    ...
    room_vplane_norm_vector{i}(1,1:3)=[-sign(d1-d2)*a/sqrt(a^2+b^2+c^2),-sign(d1-d2)*b/sqrt(a^2+b^2+c^2),-sign(d1-d2)*c/sqrt(a^2+b^2+c^2)];
end
%% fourthly, adding the above result into the renovation_planes
plane_num_now=size(renovation_planes_edge_cell,2);
for i=1:1:size(room_vplane_edge_cell,2)
    new_room_planes_edge_cell{i+plane_num_now}=room_vplane_edge_cell{i};
