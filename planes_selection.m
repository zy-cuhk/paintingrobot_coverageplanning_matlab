...
for i=1:1:size(renovation_planes_edge_cell,2)
    ...
    if i==6
        ...
        for j=1:1:size(room_effective_waypoints{i},1)
            renovation_effective_waypoints(j,1)=(room_effective_waypoints{i}(j,1)-sign(d1-d2)*a/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
            renovation_effective_waypoints(j,2)=(room_effective_waypoints{i}(j,2)-sign(d1-d2)*b/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
            renovation_effective_waypoints(j,3)=(room_effective_waypoints{i}(j,3)-sign(d1-d2)*c/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
            renovation_effective_waypoints(j,4:6)=[theta_x,theta_y(i),theta_z];
        end
        for j=1:1:size(room_effective_waypaths{i},1)
            for k=1:1:2
                renovation_effective_waypaths(j,3*k-2)=(room_effective_waypaths{i}(j,3*k-2)-sign(d1-d2)*a/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
                renovation_effective_waypaths(j,3*k-1)=(room_effective_waypaths{i}(j,3*k-1)-sign(d1-d2)*b/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
                renovation_effective_waypaths(j,3*k)=(room_effective_waypaths{i}(j,3*k)-sign(d1-d2)*c/sqrt(a^2+b^2+c^2)*panning_distance)*0.001;
            end
        end
        m=m+1;
    end
