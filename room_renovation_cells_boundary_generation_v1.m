...
for i=1:1:size(renovation_cells_manipulatorbase_positions,2)
    for j=1:1:size(renovation_cells_manipulatorbase_positions{i},1)
        for k=1:1:size(renovation_cells_manipulatorbase_positions{i}{j},2)
            z0=renovation_cells_manipulatorbase_positions{i}{j}{k}(1,3);
            if z0>1.20
                renovationcells_pathswaypoints_classified1{i}{j}{m}=renovationcells_pathswaypoints_classified{i}{j}{k};
                renovation_cells_manipulatorbase_positions1{i}{j}{m}=renovation_cells_manipulatorbase_positions{i}{j}{k};
                manipulator_endeffector_positions1{i}{j}{m}=manipulator_endeffector_positions{i}{j}{k};
                renovation_cells_edges_classified1{i}{j}{m}=renovation_cells_edges_classified{i}{j}{k};
                renovation_cells_paths_classified1{i}{j}{m}=renovation_cells_paths_classified{i}{j}{k};
                m=m+1;
            end
        end
    end
...
% input: renovation_cells_paths{:}{:}(:,1:6)
% input: renovation_cells_edges{:}{:}(:,1:6)
% input: renovation_planes_norm_vector{:}(1,1:3)
% input: renovation_planes_edge_cell{}(:,1:6)
% input: room_verticalplanes_edge_cell{:}(:,1:6)
% input: renovation_manipulatorbase_planes{}(:,1:3)

% output: renovation_cells_paths_classified{:}{:}{:}(:,1:6)
% output: renovation_cells_edges_classified{:}{:}{:}(:,1:6)
% output: renovation_planes_norm_vector{}(1,1:3)
% output: renovation_planes_edge_cell{}(:,1:6)
% output: room_verticalplanes_edge_cell{:}(:,1:6)
% ouput: renovation_manipulatorbase_planes{:}(1,1:3)


% for i=1:1:size(new_renovation_planes_edge_cell,2)
%     n=1; xy_vector=[];
%     for j=1:1:size(new_renovation_planes_edge_cell{i},1)
%         xy_vector(n,1)=new_renovation_planes_edge_cell{i}(j,1);
%         xy_vector(n,2)=new_renovation_planes_edge_cell{i}(j,2);
%         n=n+1;
%     end
%     xy_vector=unique(xy_vector,'rows');
%     xy_vector=sortrows(xy_vector);
%     room_renovationplaneedges_endpoints(i,1:2)=xy_vector(1,1:2);
%     room_renovationplaneedges_endpoints(i,3:4)=xy_vector(end,1:2);
% end
% 
% i=1;
% while(1)
%     p1=room_renovationplaneedges_endpoints(i,1:2);
%     p2=room_renovationplaneedges_endpoints(i,3:4);
% %     for i=
% %         if room_renovationplaneedges_endpoints
% %        
% %    end
% end
